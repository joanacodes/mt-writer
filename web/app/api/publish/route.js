...
import { putFile, b64 } from '@/lib/github';
...
export async function POST(req) {
  ...
  for (const id of ids) {
    ...
    try {
      for (const a of arts || []) {
        const path = a.lang === 'en' ? `content/en/blog/${a.slug}.md` : `content/fr/blog/${a.slug}.md`;
        await putFile(path, b64(a.body), `Article: ${row.title_en || row.title_fr} (${a.lang})`);
        await db.from('articles').update({ published_at: new Date().toISOString() }).eq('plan_id', id).eq('lang', a.lang);
      }
      if (cover?.data) {
        await putFile(`assets/covers/${cover.slug}.jpg`, cover.data, `Cover: ${cover.slug}`);
